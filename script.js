...
// import { getFirestore, collection, getDocs, doc, setDoc, getDoc, query, where } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
...
// // --- MODAL HANDLERS --- (Không thay đổi)
// const openPublicProfileModal = async (userId) => {
//     const profDocRef = doc(db, "professionals", userId);
//     const userDocRef = doc(db, "users", userId);
    
//     const [profDoc, userDoc] = await Promise.all([getDoc(profDocRef), getDoc(userDocRef)]);
...
//                 <img src="${user.photoURL}" class="w-20 h-20 rounded-full object-cover"/>
//                 <div>
//                     <h3 class="text-lg font-bold">${user.displayName}</h3>
//                     <p class="font-bold text-gray-800 capitalize">${prof.service} - $${prof.price}/hr</p>
//                     <div class="flex items-center mt-1">
//                         ${createStarRating(prof.averageRating || 0)}
//                         <span class="text-xs text-gray-500 ml-2">(${(prof.reviewCount || 0)} reviews)</span>
//                     </div>
...
//                 <div>
//                     <p class="font-semibold">${booking.talentName}</p>
//                     <p class="text-sm text-gray-500">${new Date(booking.createdAt.seconds * 1000).toLocaleDateString()}</p>
//                 </div>
//                 <div class="flex items-center space-x-2">
//                      <p class="text-sm font-semibold capitalize px-2 py-1 rounded-full ${booking.status === 'completed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}">${booking.status}</p>
...
//             </div>
//         `).join('');
//         bookingsListContainer.innerHTML = bookingsListHtml;
//     } else {
//         bookingsListContainer.innerHTML = '<p class="text-gray-500">You have no bookings.</p>';
//     }
...
// // --- DATA HANDLING FUNCTIONS --- (Không thay đổi)
...
//         lat: coords.lat, // Đây là kiểu number
//         lng: coords.lng, // Đây là kiểu number
//     };
...
//     // Vô hiệu hóa nút và hiển thị trạng thái đang tìm kiếm
...
//     try {
//         // BƯỚC 1: Tải danh sách talent MỚI NHẤT từ Firestore mỗi khi tìm kiếm
//         console.log("Đang tải danh sách talent từ Firestore...");
...
//         console.log(`Tải thành công ${allProfessionals.length} talent.`);

//         // BƯỚC 2: Lấy tọa độ (lat, lng) của địa điểm người dùng nhập
//         const response = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(location)}&format=json&limit=1`, {
//             headers: { 'User-Agent': 'TravelTalentApp/1.0 (your-email@example.com)' } // Thay bằng email của bạn
//         });
...
//             console.log(`Kết quả: Tìm thấy ${finalFilteredList.length} talent phù hợp.`);
...
//     } finally {
//         // Khôi phục lại nút tìm kiếm sau khi hoàn tất
//         searchBtn.disabled = false;
//         searchBtn.innerHTML = '<i data-lucide="search" class="w-5 h-5"></i><span>Search</span>';
//         lucide.createIcons();
//     }
...
//     }
//     // Chỉ cần render bản đồ và danh sách rỗng khi mới vào trang
//     renderProfessionals([]);
//     renderMap([]);
// });
...
import { initializeApp } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-app.js";
import { getAuth, GoogleAuthProvider, signInWithPopup, signOut, onAuthStateChanged } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-auth.js";
// NEW: Import updateDoc
import { getFirestore, collection, getDocs, doc, setDoc, getDoc, query, where, updateDoc } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
...
const openMyProfileModal = async () => {
    ...
    const [userDoc, profDoc] = await Promise.all([getDoc(userDocRef), getDoc(profDocRef)]);
    ...
    // Setup event listeners for the booking list
    const bookingsListContainer = document.getElementById('bookings-list');
    bookingsListContainer.addEventListener('click', async (e) => {
        ...
        if (e.target.classList.contains('accept-btn')) {
            ...
            loadAndRenderBookings(user); // Refresh the list
        }
